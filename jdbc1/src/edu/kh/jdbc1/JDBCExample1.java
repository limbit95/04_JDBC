...
public class JDBCExample1 {
	...
	public static void main(String[] args) {
		...
		//    JDBC를 상속받아 구현할 클래스 모음 (ojdbc.jar 라이브러리)
		
		
		
		// 1단계 : JDBC 객체 참조 변수선언 (java.sql 패키지의 인터페이스)
		
		Connection conn = null;
		// DB 연결 정보를 담은 객체
		// -> DBMS 타입, 이름, IP, Port, 계정명, 비밀번호
		// -> DBeaver의 계정 접속 방법가 유사함
		...
		try{
			...
			// 1522 (원래 기본값은 1521인데 내 컴퓨터 한해서는 1522)
			
			String sid = ":XE"; // DB 이름
			
			//url == jdbc:oracle:thin:@localhost:1522:XE
			
			String user = "kh_lsh"; // 사용자 계정
			...
			// url 인자 값으로 전달 == jdbc:oracle:thin:@localhost:1522:XE
			
			// 중간확인
//			System.out.println(conn); // oracle.jdbc.driver.T4CConnection@72d1ad2e
			
			// 3. SQl 작성
			// *** Java에서 작성되는 SQL 쿼리문은 마지막에 ; 을 작성하면 안됨 ***
